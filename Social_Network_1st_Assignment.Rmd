...
```{r}
## AU: I'm trying to create a new code since the previous one gives me an error
...
# Extract Venom's Neighborhood
venom_neighborhood <- ego(marvel_network, order = 1, nodes = venom_node, mode = "all")[[1]]

# Create Venom's Subgraph
venom_subgraph <- induced_subgraph(marvel_network, venom_neighborhood)

# Map Character IDs to Names
V(venom_subgraph)$label <- marvel_nodes$character_name[match(V(venom_subgraph)$name, marvel_nodes$id)]

# Determine Node Groups
node_groups <- marvel_nodes$group[match(V(venom_subgraph)$name, marvel_nodes$id)]

# Map Group Values to Colors
# Ensure node_groups is a numeric vector
...
# Plot the Subgraph
plot(venom_subgraph,
     layout = layout_with_fr(venom_subgraph),  # Ensure layout is correctly defined
     vertex.label = V(venom_subgraph)$label,
     vertex.label.cex = 0.8,
     vertex.label.dist = 1.2,
     vertex.label.degree = pi/4,
