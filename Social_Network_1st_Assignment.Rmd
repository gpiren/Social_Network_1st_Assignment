...
The results show Venom, Doctor Doom, Spider-Man, Puppet Master, Krang, and Attuma as the top 6, meaning they are key bridges in the network, frequently connecting other characters.
...
#### Calculate the centrality of the characters in the Marvel network using the degree, betweenness and page rank.
...
*!!!! The harmonic centrality scores are added to your existing centrality_df data frame, allowing you to compare them with other centrality measures.*
...
To visually compare the different centrality measures, we can create a series of plots that highlight how each measure ranks the characters in your network.
These visualizations will help us gain a comprehensive understanding of how different centrality measures rank the characters in our network and how these measures relate to each other.


*Scatter Plot Matrix*
A scatter plot matrix can help us compare pairs of centrality measures to see how they correlate with each other. This can be done using the GGally package, which provides a convenient way to create pairwise scatter plots.
```{r}
library(GGally)

# Select relevant columns for the scatter plot matrix
centrality_df_melted <- melt(centrality_df, id.vars = "character_name")
...
*Radar Chart*
A radar chart can be used to visualize the centrality measures for the top characters across all metrics. This allows you to see which characters are consistently ranked high across different measures.
```{r}
library(fmsb)
